...
# ========== Define constants
DATASETS = ["liugan", "combined_samples", "MirTarRAW"]
...
parser.add_argument('--model_name_or_path', type=str,
                    default="/home/shenweiwei/CatIIIIIIII-RNAErnie-faa2b2d/output/BERT,ERNIE,MOTIF,PROMPT/checkpoint_final",
...
parser.add_argument('--model_path',
                    type=str,
                    default="/home/shenweiwei/CatIIIIIIII-RNAErnie-faa2b2d/output/BERT,ERNIE,MOTIF,PROMPT/checkpoint_final/model_state.pdparams",
...
# ... (前面的代码保持不变，直到 if __name__ == "__main__": 之后的部分)
...
if __name__ == "__main__":
    ...
    logger.info("Start training." if args.train else "Start evaluation.")
    ...
    if args.train:
        ...
        # evaluate dataset only and plot ROC curve
        _, outputs, labels, _ = rr_inter_trainer.eval(0)

        # Convert outputs to probabilities and predictions
        probs = paddle.nn.functional.softmax(outputs, axis=1).numpy()
        ...
        roc_path = os.path.join(args.output, "roc_curve_cv.pdf")
        plt.savefig(roc_path, format='pdf', dpi=800, bbox_inches='tight')
        plt.close()
        print("ROC curve saved to {}".format(roc_path))
