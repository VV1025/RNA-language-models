...
class RRInterMetrics(BaseMetrics):
    ...
    def __call__(self, outputs, labels):
        ...
        labels = paddle.cast(labels, dtype='int64').flatten()

        # Calculate TP, TN, FP, FN
        tp = paddle.sum((preds == 1) & (labels == 1)).item()
        tn = paddle.sum((preds == 0) & (labels == 0)).item()
        fp = paddle.sum((preds == 1) & (labels == 0)).item()
        fn = paddle.sum((preds == 0) & (labels == 1)).item()

        # Add to metrics
        metrics = super().__call__(outputs, labels)
        metrics['TP'] = tp
        metrics['TN'] = tn
        metrics['FP'] = fp
        metrics['FN'] = fn
...
class RRInterTrainer(BaseTrainer):
    ...
    def eval(self, epoch):
        ...
        Returns:
            tuple: (names, outputs, labels, attn) if return_preds is True, else None
        """
        self.model.eval()
        time_st = time.time()
