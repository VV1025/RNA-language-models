...
# ========== Define constants
STRATEGIES = ["BERT", "ERNIE", "MOTIF", "PROMPT"]
...
parser.add_argument('--model_type', type=str, default="ernie", help='The build-in large model type.')
...
parser.add_argument('--dataset_dir', type=str, default="./data/pre_random", help='Local path for dataset.')
parser.add_argument('--dataset_name', type=str, default="group", help='Local path for dataset.')
...
parser.add_argument('--vocab_size', type=int, default=100, help='The size of vocobulary.')
parser.add_argument('--masked_lm_prob', type=float, default=0.15, help='Mask token probability.')
parser.add_argument('--pre_strategy', type=str2list, default="BERT",
                    help='Masking strategies, can accept concatenated ones with ","')
parser.add_argument('--num_consumed_samples', type=int, default=0, help='Total consumed pretrain examples number.')
parser.add_argument('--num_samples_per_file', type=int, default=300000, help='Examples number in 1 fasta group file.')
parser.add_argument('--num_groups', type=int, default=86, help='Total pretrain group file number.')
parser.add_argument('--motif_dir', type=str, default="./data/motif", help='Motif file root directory.')
parser.add_argument('--motif_files', type=str, default="ATtRACT,SpliceAid,Statistics", help='Motif files to use.')
...
parser.add_argument('--dataloader_num_workers', type=int, default=50, help='The number of threads used by dataloader.')
...
parser.add_argument('--num_train_epochs', type=int, default=1, help='The number of epoch for training.')
parser.add_argument('--max_steps', type=int, default=-1, help='Maximum steps, which overwrites num_epoch.')

parser.add_argument('--warmup_ratio', type=float, default=0.02, help='Linear warmup over warmup_ratio fraction of '
                                                                     'total steps.')
parser.add_argument('--decay_steps', type=int, default=-1,
                    help='The steps for learning rate decay, default to total steps.')
parser.add_argument('--min_learning_rate', type=float, default=1e-4, help='Minimal learning rate after decay.')
parser.add_argument('--learning_rate', type=float, default=1e-4, help='The learning rate of optimizer.')

parser.add_argument('--save_steps', type=int, default=1000, help='Save model every save_steps steps.')
parser.add_argument('--save_total_limit', type=int, default=50, help='Maximum saving times.')
parser.add_argument('--logging_steps', type=int, default=100, help='Print logs every logging_step steps.')
parser.add_argument('--report_to', type=list, default=["visualdl"], help='The list of integrations to report the '
                                                                         'results and logs to.')
