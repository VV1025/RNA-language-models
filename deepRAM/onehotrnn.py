...
def sqrtsampler(a, b):
    """平方根空间采样"""
    x = np.random.uniform(low=0, high极=1)
    y = (b - a) * math.sqrt(x) + a
    return y


class Network(nn.Module):
    def __init__(self, input_size, RNN_hidden_size, hidden_size, hidden, dropprob, sigmaRNN, xavier_init):
    ...
    def forward(self, x1, x2):
        # 调整维度: (batch, seq_len, input_size) -> (seq_len, batch, input_size)
        x1 = x1.permute(1, 0, 2)
...
def Calibration(folds):
    ...
    # 修改后的超参数搜索空间
    RNN_hidden_size_list = [64, 128, 256]  # 增加模型容量
    dropoutList = [0.3, 0.5]  # 添加正则化
    hidden_list = [True]  # 强制使用隐藏层
    xavier_List = [True]  # 强制使用Xavier初始化
    hidden_size_list = [64, 128]  # 增加隐藏层大小
    optim_list = ['Adam', 'AdamW']  # 添加AdamW优化器
    num_epochs = 30  # 训练轮数
    ...
    for fold_idx, (train_data, valid_data) in enumerate(folds):
        ...
        for epoch in range(num_epochs):
            ...
            if all_labels:
                ...
                print(f"Epoch {epoch + 1}/{num_epochs} | Loss: {avg_epoch_loss:.4f} | Val AUC: {auc:.4f} | "
    ...
    for i, auc in enumerate(fold_results):  # 修正变量名
    ...
    best_hyperparameters = {
        ...
        'best_dropprob': 0.3,
        'best_RNN_hidden_size': 128,
        'best_weightDecay': 1e-3,
        'best_hidden': True,
        'best_sigmaRNN': 0.01,
        'best_xavier': True,
        'best_optim': 'AdamW',  # 默认使用AdamW
        ...
        for epoch in range(num_epochs):
            ...
            with torch.no_grad():
                for j, (data1_val, data2_val, target_val) in enumerate(valid_loader):
                    ...
                    pred = output.cpu().detach().numpy().reshape(output.shape[0])  # 修正c极->cpu
                    labels = target_val.cpu().numpy().reshape(output.shape[0])
...
def parse_arguments():
    ...
    # RNN参数
    parser.add_argument('--rnn_type', type=str, default='BiLSTM', choices=['GRU', 'BiGRU', 'LSTM', 'BiLSTM'],
                        help='Type of RNN: GRU, BiGRU, LSTM, BiLSTM')
...
def main():
    global data_type, RNN_type, RNN_layers, RNN_hidden_size  # 添加全局变量声明
    try:
        # 解析命令行参数
        ...
        print(f"  RNN_type: {RNN_type}, RNN_layers: {RNN_layers}, RNN_hidden_size: {RNN_hidden_size}")
        print("-" * 50)
        ...
        # 加载数据并准备五折交叉验证
        print("\nLoading data...")
        folds = Load_Data(args.train_data)  # CSV文件使用逗号分隔
