...
class Network(nn.Module):
    def __init__(self, embedding_dim, num_filters, filter_sizes, hidden_size, hidden, dropprob, xavier_init,
                 w2v_weights=None):
        ...
        if w2v_weights is not None:
            ...
            vocab_size = len(bases) ** kmer_size + 2
            self.embedding = nn.Embedding(
                vocab_size,
                embedding_dim,
                padding_idx=0
        ...
        # 设置全连接层大小
        self.FC_size = num_filters * len(filter_sizes) * 2  # 每个序列的每种卷积核大小都有一个输出
        ...
        for conv in self.convs:
            if xavier_init:
                ...
                nn.init.normal_(conv.weight, mean=0, std=0.05)
        ...
        if xavier_init:
            ...
            if hidden:
                ...
                nn.init.normal_(self.fc.weight, mean=0, std=0.05)
    ...
    def forward(self, x1, x2):
        ...
        # 调整维度为卷积需要的格式 (batch, channels, seq_len)
        x1 = x1.permute(0, 2, 1)
        x2 = x2.permute(0, 2, 1)
        ...
        conv_outputs1 = []
        for conv in self.convs:
            conv_out = conv(x1)  # (batch, num_filters, seq_len)
            conv_out = self.pool(conv_out)  # (batch, num_filters, 1)
            conv_out = F.relu(conv_out).squeeze(-1)  # (batch, num_filters)
            conv_outputs1.append(conv_out)

        x1 = torch.cat(conv_outputs1, dim=1)  # (batch, num_filters * len(filter_sizes))
        x1 = self.dropout(x1)
        ...
        for conv in self.convs:
            ...
            conv_out = F.relu(conv_out).squeeze(-1)
            conv_outputs2.append(conv_out)

        x2 = torch.cat(conv_outputs2, dim=1)
        x2 = self.dropout(x2)

        # 合并两个序列的特征
        x = torch.cat((x1, x2), dim=1)  # (batch, 2 * num_filters * len(filter_sizes))
...
def Calibration(folds, w2v_model):
    ...
    for fold_idx, (train_data, valid_data) in enumerate(folds):
        ...
        print(f"  Num filters: {num_filters}, Filter sizes: {filter_sizes}, Dropout: {dropprob}")
    ...
    best_hyperparameters = {
        ...
        'best_num_filters': best_hparams['num_filters'],
        'best_filter_sizes': best_hparams['filter_sizes'],
...
def Train_model(folds, best_hyperparameters, model_dir, model_path, out_file, w2v_model):
    ...
    best_num_filters = best_hyperparameters['best_num_filters']
    best_filter_sizes = best_hyperparameters['best_filter_sizes']
    ...
    for fold_idx, (train_data, valid_data) in enumerate(folds):
        ...
        model = Network(
            ...
            filter_sizes=best_filter_sizes,
...
def main():
    parser = argparse.ArgumentParser(description='Sequence Interaction Prediction with CNN and Word2Vec Embeddings')
