...
class HybridNetwork(nn.Module):
    def __init__(self, embedding_dim, RNN_hidden_size, hidden_size, hidden, dropprob, sigmaRNN, xavier_init,
                 w2v_weights=None, cnn_out_channels=64, cnn_kernel_size=3):
        ...
        self.conv1d = nn.Conv1d(
            ...
            padding=cnn_kernel_size // 2  # 保持序列长度不变
        )
        ...
        # 初始化CNN权重
        if not xavier_init:
            torch.nn.init.normal_(self.conv1d.weight, mean=0, std=sigmaRNN)
            torch.nn.init.normal_(self.conv1d.bias, mean=0, std=sigmaRNN)
        else:
    ...
    def forward(self, x1, x2):
        ...
        # 调整维度: (batch, seq_len, emb_dim) -> (batch, emb_dim, seq_len)
        x1 = x1.permute(0, 2, 1)
        x2 = x2.permute(0, 2, 1)
        ...
        # 调整维度: (batch, channels, seq_len) -> (seq_len, batch, channels)
        x1 = x1.permute(2, 0, 1)
...
def Calibration(folds, w2v_model):
    ...
    # CNN相关参数
    cnn_out_channels_list = [32, 64, 128]
    cnn_kernel_size_list = [3, 5, 7]
    ...
    for fold_idx, (train_data, valid_data) in enumerate(folds):
        ...
        # CNN参数采样
        cnn_out_channels = random.choice(cnn_out_channels_list)
        cnn_kernel_size = random.choice(cnn_kernel_size_list)
        ...
        print(f"  CNN out channels: {cnn_out_channels}, CNN kernel size: {cnn_kernel_size}")
    ...
    best_hyperparameters = {
        ...
        'best_cnn_out_channels': best_hparams['cnn_out_channels'],
        'best_cnn_kernel_size': best_hparams['cnn_kernel_size']
    }
...
def Train_model(folds, best_hyperparameters, model_dir, model_path, out_file, w2v_model):
    ...
    best_cnn_out_channels = best_hyperparameters['best_cnn_out_channels']
    best_cnn_kernel_size = best_hyperparameters['best_cnn_kernel_size']
    ...
    for fold_idx, (train_data, valid_data) in enumerate(folds):
        ...
        print(f"Using CNN configuration: out_channels={best_cnn_out_channels}, kernel_size={best_cnn_kernel_size}")
        ...
        model = HybridNetwork(
            ...
            w2v_weights=w2v_weights,
            cnn_out_channels=best_cnn_out_channels,
            cnn_kernel_size=best_cnn_kernel_size
        ).to(device)
...
def main():
    parser = argparse.ArgumentParser(description='Sequence Interaction Prediction with Hybrid CNN-RNN Model')
